...
Thomas.prototype.setup = function(canvas_id) {
	if(canvas_id) {
		...
		//TODO: make separate buffer class
	  	earth = new Entity(new vec2(0.0, 0.0), new vec2(100.0, 100.0));

		earth.createRenderable(this.gl, this.programs["default"])
		earth.renderable.addPrimitive(twgl.primitives.createSphereVertices(1.0, 32.0, 32.0));
		
		earth.renderable.applyCustomUniforms([
			["tex", this.textures.earth]
		]);
	
		earth.renderable.setCustomUniforms();
		earth.finalizeBuffers(this.gl);

		// for(var i = 0; i < 10; i++) {
		// 	var testEntity = new Entity(new vec2(0, 0), new vec2(100.0, 100.0));
		// 	testEntity.bindRenderable(earth.renderable);
...
Thomas.prototype.render = function() {
	...
	mat4.identity(earth.renderable.model);
	mat4.translate(earth.renderable.model, earth.renderable.model, [earth.position.x, 0, earth.position.y]);
	mat4.rotateY(earth.renderable.model, earth.renderable.model, jf);
	earth.render();
